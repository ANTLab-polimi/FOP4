...
class P4DockerHost(Docker):
    def __init__(self, name, **params):
        Docker.__init__(self, name, **params)

    def config(self, **params):
        r = super(Docker, self).config(**params)
        for off in ["rx", "tx", "sg"]:
            cmd = "/sbin/ethtool --offload %s %s off" \
                  % (self.defaultIntf(), off)
            self.cmd(cmd)
...
class ONOSBmv2Switch(Switch):
    ...
    def doOnosNetcfg(self, controllerIP):
        ...
        if not srcIP:
            warn("*** WARN: unable to get controller IP address, won't do netcfg\n")
            return
