...
control ingress (
    ...
    apply {
        ...
        table0_control.apply(hdr, local_metadata, standard_metadata); // --> Devo settare tabelle qui dentro (table0)
        process_int_source_sink.apply(hdr, local_metadata, standard_metadata); // --> Devo settare tabelle qui dentro (tb_set_source, tb_set_sink)

        if (local_metadata.int_meta.source == _TRUE) {
            process_int_source.apply(hdr, local_metadata, standard_metadata); // --> Devo settare tabelle qui dentro (tb_int_source)
        }
...
control egress (
    ...
    apply {
        if(hdr.int_header.isValid()) {
            process_int_transit.apply(hdr, local_metadata, standard_metadata); // --> Devo settare tabelle qui dentro (tb_int_insert)

            #ifdef TARGET_BMV2
            if (IS_I2E_CLONE(standard_metadata)) {
                /* send int report */
                process_int_report.apply(hdr, local_metadata, standard_metadata); // --> Devo settare tabelle qui dentro (tb_generate_report)
            }
